...
export default function DetalleModal({
  ...
  const handleAnular = async () => {
    if (
      !confirm(
        `¿Anular la factura #${detalle.numero}? Esta acción es irreversible.`,
      )
    )
      return;
    setAnulando(true);
    try {
      await onAnular!(detalle.id);
      showToast("Factura anulada");
      onClose();
    } catch (e: any) {
      showToast(e.message ?? "Error al anular");
    } finally {
      setAnulando(false);
    }
  };
        ...
        <div className="modal w-[90%] max-w-xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              {detalle.anulada && (
                <span className="text-xs font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-red-100 text-red-600">
                  Anulada
                </span>
              )}
            </div>
          ...
          <div className="flex flex-col gap-4 text-sm">
                  ...
                  <table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                      {detalle.lineas.map((l: any) => (
                        <tr
                          key={l.id}
                          className="border-b border-[var(--cream)]"
                        >
            ...
            {/* FECHA ANULACIÓN */}
            {detalle.anulada && detalle.anuladaAt && (
              <div className="text-xs text-red-500 text-center pt-2 border-t border-[var(--cream)]">
                Anulada el{" "}
                {new Date(detalle.anuladaAt).toLocaleDateString("es-AR")}
              </div>
            )}
          </div>
        </div>
