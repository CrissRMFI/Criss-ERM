...
const TIPOS_PAGO = ['Efectivo','Transferencia','Tarjeta débito','Tarjeta crédito','Cheque','QR / Billetera','Otro']

function fmt(n: number) { return '$ ' + (isNaN(n) ? '0.00' : n.toFixed(2)) }
...
let _rowId = 0
const newFila = (): FilaProducto => ({ id: `r${_rowId++}`, nombre: '', precio: 0, qty: 1 })
let _payId = 0
const newPago = (): Pago => ({ id: `p${_payId++}`, tipo: TIPOS_PAGO[0], detalle: '', monto: 0 })

// ── Fila de producto ─────────────────────────────────────────────────────────
function FilaRow({
  fila, onChange, onDelete,
}: {
  fila: FilaProducto
  ...
  return (
    <tr>
      <td>
        <div className="prod-wrap" ref={ddRef}>
          <input
            ...
            onFocus={() => { search(query); setDdOpen(true) }}
          ...
          {ddOpen && (
            <div className="dd open">
              {results.length === 0
                ...
                : results.map(p => (
                  <div key={p.id} className="dd-row" onMouseDown={() => pick(p)}>
                    <span>{p.nombre}</span>
      ...
      <td className="c">
        <input
          className="td-in qty"
          type="number" min={1} value={fila.qty}
          onChange={e => onChange({ ...fila, qty: Math.max(1, parseInt(e.target.value)||1) })}
        />
      </td>
      <td className="r">
        <input
          ...
          value={fila.precio || ''}
          onChange={e => onChange({ ...fila, precio: parseFloat(e.target.value)||0 })}
        />
      </td>
      <td><span className="row-sub">{fmt(subtotal)}</span></td>
      <td><button className="del-row" onClick={onDelete}>✕</button></td>
    </tr>
  )
}

// ── Fila de pago ─────────────────────────────────────────────────────────────
function PagoItem({ pago, onChange, onDelete }: {
  pago: Pago; onChange: (p: Pago) => void; onDelete: () => void
}) {
  return (
    <div className="pay-item">
      <select value={pago.tipo} onChange={e => onChange({ ...pago, tipo: e.target.value })}>
        {TIPOS_PAGO.map(t => <option key={t}>{t}</option>)}
      </select>
      <input
        type="text" className="pay-detail" placeholder="Detalle…"
        value={pago.detalle}
        onChange={e => onChange({ ...pago, detalle: e.target.value })}
      />
      <input
        type="number" className="pay-amount" placeholder="0.00" step="0.01"
        value={pago.monto || ''}
...
export default function FacturaPage() {
  ...
  const [filas, setFilas] = useState<FilaProducto[]>([newFila(), newFila(), newFila()])
  const [saldoAnt, setSaldoAnt] = useState<number|''>('')
  const [pagos, setPagos] = useState<Pago[]>([])
  const [cajaDatos, setCajaDatos] = useState({ deuda:'', dejo:'', retiro:'' })
  ...
  const capture = async () => {
    ...
    return html2canvas(cardRef.current!, { scale: 2, useCORS: true, backgroundColor: '#ffffff', logging: false })
  }
  ...
  const exportPDF = async () => {
    ...
    pdf.save(`factura_${nro}_${cliente.replace(/\s+/g,'_') || 'cliente'}_${fecha}.pdf`)
  }
  ...
  return (
    <>
      ...
      <div className="card" id="invoice-card" ref={cardRef}>
        ...
        <div className="inv-header">
          ...
          <div className="meta-grid">
            <div className="meta-item">
              ...
              <input type="text" className="meta-input nro-input" value={nro} onChange={e => setNro(e.target.value)} />
            </div>
            <div className="meta-item">
              <span className="meta-label">Fecha</span>
              <input type="date" className="meta-input" value={fecha} onChange={e => setFecha(e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="inv-body">
          ...
          <div>
            ...
            <input
              type="text" className="field-input" placeholder="Nombre completo…"
              ...
              <tbody>
                {filas.map(f => (
                  <FilaRow
                    key={f.id}
                    fila={f}
                    onChange={updated => setFilas(fs => fs.map(x => x.id === updated.id ? updated : x))}
                    onDelete={() => setFilas(fs => fs.filter(x => x.id !== f.id))}
                  />
                ))}
              </tbody>
            </table>
            <button className="add-row-btn" onClick={() => setFilas(fs => [...fs, newFila()])}>
              + Agregar producto
            </button>
          </div>
          ...
          <div>
            ...
            <div className="totals-box">
              ...
              <div className="t-row">
                ...
                <input
                  type="number" className="t-inp" placeholder="0.00" step="0.01"
                  value={saldoAnt}
                  onChange={e => setSaldoAnt(e.target.value === '' ? '' : parseFloat(e.target.value))}
                />
              </div>
            ...
            <div className="pay-list">
              {pagos.map(p => (
                <PagoItem
                  key={p.id}
                  pago={p}
                  onChange={updated => setPagos(ps => ps.map(x => x.id === updated.id ? updated : x))}
                  onDelete={() => setPagos(ps => ps.filter(x => x.id !== p.id))}
                />
              ))}
            </div>
            <button className="add-pay-btn" onClick={() => setPagos(ps => [...ps, newPago()])}>
              + Agregar pago
            </button>
            <div className="totals-box" style={{ marginTop: 12 }}>
              <div className="t-row">
                <span className="t-lbl">Total Pagado</span>
                <span className="t-val" style={{ color: 'var(--sage)' }}>{fmt(totalPagado)}</span>
              </div>
              <div className="t-row grand">
                <span className="t-lbl">Saldo Pendiente</span>
                <span className="t-val" style={{ color: saldoPendiente > 0.005 ? 'var(--rust)' : 'var(--sage)' }}>
                  {fmt(saldoPendiente)}
                </span>
              </div>
            </div>
              ...
              {(['deuda','dejo','retiro'] as const).map(k => (
                <div className="caja" key={k}>
                  ...
                  <input
                    ...
                    value={cajaDatos[k]}
                    onChange={e => setCajaDatos(c => ({ ...c, [k]: e.target.value }))}
                  />
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea
              className="field-input" placeholder="Notas, aclaraciones…"
      ...
      {/* MODAL WA */}
      <div className={`modal-bg ${waOpen ? 'open' : ''}`} onClick={e => e.target === e.currentTarget && setWaOpen(false)}>
        <div className="modal wa-modal">
          <h3>Compartir por WhatsApp</h3>
          <p>La factura se descargará como imagen PNG. Luego se abrirá WhatsApp Web para que la adjuntes y envíes.</p>
          <div className="modal-btns">
            <button className="btn btn-ghost btn-sm" onClick={() => setWaOpen(false)}>Cancelar</button>
            <button className="btn btn-wa btn-sm" onClick={exportWA}>Descargar y abrir WhatsApp</button>
          </div>
        </div>
      </div>
